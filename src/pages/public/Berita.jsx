      ...
      <section className="pb-24 px-6 md:px-12 lg:px-16 flex-grow w-full max-w-[1200px] mx-auto -mt-10 relative z-20">
        ...
        {loading ? (
          ...
          <>
            ...
            {featured && (
              <ScrollReveal>
                <Link to={`/berita/${featured.id}`} className="bg-white rounded-[32px] overflow-hidden shadow-xl shadow-gray-200/40 border border-gray-100 flex flex-col lg:flex-row group mb-12 block hover:shadow-2xl transition-all duration-500">
                  <div className="w-full lg:w-1/2 relative overflow-hidden h-[300px] lg:h-auto">
                    ...
                    <img 
                      src={featured.image || "https://images.unsplash.com/photo-1541339907198-e08756dedf3f?q=80&w=2070&auto=format&fit=crop"} 
                      alt={featured.title} 
                  ...
                  <div className="w-full lg:w-1/2 p-8 lg:p-14 flex flex-col justify-center">
                    <span className="text-[#0F4C3A] font-bold text-[13px] uppercase tracking-widest mb-4 inline-block">{featured.category || 'Kampus'}</span>
                    <h2 className="text-2xl lg:text-3xl font-extrabold text-gray-900 mb-5 leading-tight group-hover:text-[#0F4C3A] transition-colors">{featured.title}</h2>
                    <p className="text-gray-500 text-[16px] leading-relaxed mb-8">{featured.content ? featured.content.replace(/<[^>]*>?/gm, '').substring(0, 200) + '...' : ''}</p>
                    <div className="flex items-center justify-between mt-auto">
                      <div className="flex items-center gap-6">
                        <div className="flex items-center gap-2 text-[14px] text-gray-500 font-medium">
                          ...
                          {featured.created_at ? new Date(featured.created_at).toLocaleDateString('id-ID') : '-'}
                        </div>
                      </div>
                ...
                <ScrollReveal key={news.id} delay={idx * 0.1} className="h-full">
                  <Link to={`/berita/${news.id}`} className="bg-white rounded-[24px] overflow-hidden shadow-lg shadow-gray-200/40 border border-gray-100 flex flex-col h-full group block hover:-translate-y-2 hover:shadow-xl transition-all duration-300">
                    <div className="relative overflow-hidden h-56">
                      ...
                      <img 
                        src={news.image || "https://images.unsplash.com/photo-1541339907198-e08756dedf3f?q=80&w=2070&auto=format&fit=crop"} 
                        alt={news.title} 
                    ...
                    <div className="p-6 flex flex-col flex-grow">
                      ...
                      <p className="text-gray-500 text-[15px] leading-relaxed mb-6 line-clamp-3">{news.content ? news.content.replace(/<[^>]*>?/gm, '').substring(0, 150) : ''}</p>
